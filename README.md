...
| [`retrieval/`](retrieval/) | **Module 3: retrieval + prompt injection.** `rag_retrieval` library + `rag-retrieve` CLI: question → four OpenSearch channels fused client-side (RRF) → 1-hop graph expansion over the union graph of all books (facts with polarity, entity cards, provenance) → German context block + citations; guardrail (ADR-0011, amended by REQ-001: graded prompt, follow-up rule, visible truncation), OpenAI-compatible chat client with streaming and question rewriting | done, verified on both books (`retrieval/REPORT.md`: 8 ground-truth questions, 112 unit + 13 integration tests) |
