...
__all__ = ["NO_EVIDENCE_ANSWER", "ChatClient", "Completion", "answer", "rewrite_question"]
...
class ChatClient:
    ...
    def stream(
        ...
        *,
        ...
        """Yields content deltas as they arrive (SSE ``data:`` events)."""
        payload = self._payload(messages, model=model, max_tokens=max_tokens, temperature=temperature)
        payload["stream"] = True
        for event in self.client.post_sse("/chat/completions", payload):
            for choice in event.get("choices") or []:
                delta = (choice.get("delta") or {}).get("content")
                if delta:
                    yield delta
                if choice.get("finish_reason") == "length":
                    log.warning("streamed answer truncated (finish_reason=length, max_tokens=%s)", payload["max_tokens"])
...
def answer(
    ...
    *,
    ...
    """Guardrail first: weak evidence answers ``NO_EVIDENCE_ANSWER`` without a model call unless ``force``."""
